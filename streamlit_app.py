...
def grafico(dt4pr0duct__, pr0dúct0_):
    ...
    fig, ax = pl.subplots(figsize=(12, 7), dpi=200, facecolor='#f0f0f0')
    
    ax.plot(x, y, label=pr0dúct0_, color='purple', marker='o', 
            linewidth=3, markersize=8, 
            markerfacecolor='purple', markeredgecolor='purple')
    
    z = n.polyfit(x, y, 1)
    p = n.poly1d(z)
    ax.plot(x, p(x), linestyle='--', color='darkred', 
            label='Tendencia', linewidth=3)
    
    ax.set_xticks(x)
    ax.set_xticklabels([f"{row.Año}" if row.Mes == 1 else "" for row in vent4s_.itertuples()])
    
    ax.set_title("Evolución de Ventas", 
                 fontsize=10, color='#333333')
    ax.set_xlabel("Año-Mes", fontsize=13, color='#555555')
    ax.set_ylabel("Unidades Vendidas", fontsize=12, color='#555555')
...
def main():
    ...
    if csv4rch__:
        ...
        for pr0dúct0_ in próduct_0:
            ...
            with metric4col2:
                fig = grafico(dt4pr0duct__, pr0dúct0_)
                s.pyplot(fig)
